...
import LatestFromClinic from './components/LatestFromClinic';
...
export default function App() {
  ...
  // Helper to render pages dynamically based on activePage state
  const renderPageContent = () => {
    switch (activePage) {
      ...
      case 'services':
        return <ServicesPage setActivePage={setActivePage} setSelectedServiceId={setSelectedServiceId} />;
      case 'service-single':
        return (
          <ServiceSinglePage
      ...
      case 'results':
        return <ResultsPage setActivePage={setActivePage} />;
      case 'booking':
        return <BookingPage />;
      case 'blog':
        return <BlogPage />;
      ...
      default:
        return (
          <div className="py-32 text-center text-slate-500 text-xs sm:text-sm">
            Page not found. Click the brand logo above to return home.
          </div>
        );
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FBFF] dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors duration-300 antialiased font-sans">
      ...
      {/* 5b. Floating MetaWave Innovations Proposal Badge (Bottom-Left) */}
      <div className="fixed bottom-6 left-6 z-40 no-print">
        <button
          onClick={() => {
            setActivePage((prev) => (prev === 'proposal' ? 'home' : 'proposal'));
            window.scrollTo({ top: 0, behavior: 'smooth' });
          }}
          className={`flex items-center gap-2 p-3 sm:p-3.5 rounded-full shadow-xl transition-all duration-300 hover:scale-105 active:scale-95 group cursor-pointer border ${
            activePage === 'proposal'
              ? 'bg-slate-900 border-slate-800 text-white dark:bg-white dark:text-slate-900 dark:border-white'
              : 'bg-white border-emerald-500/30 text-slate-800 dark:bg-slate-900 dark:border-emerald-500/20 dark:text-white'
          }`}
          title="Toggle Proposal Presentation"
        >
          <div className="relative flex items-center justify-center">
            <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-emerald-500 rounded-full animate-ping" />
            <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-emerald-500 rounded-full" />
            
            {/* Minimal green chevron arrow upwards */}
            <svg className="w-5 h-5 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
              <path d="M17 11l-5-5-5 5" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M17 17l-5-5-5 5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
          <span className="max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-300 ease-out text-xs font-black uppercase tracking-wider pr-0 group-hover:pr-2 whitespace-nowrap font-sans">
            {activePage === 'proposal' ? 'Exit Proposal' : 'MetaWave Proposal'}
          </span>
        </button>
      </div>
